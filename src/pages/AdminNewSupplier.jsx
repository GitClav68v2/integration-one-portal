...
export default function AdminNewSupplier() {
  ...
  useEffect(() => {
    async function suggestVendorNumber() {
      ...
      if (data && data.length > 0) {
        const last = data[0].vendor_number // e.g. VND-0001
        const num = parseInt(last.replace('VND-', ''), 10)
        setForm(f => ({ ...f, vendor_number: `VND-${String(num + 1).padStart(4, '0')}` }))
      } else {
        setForm(f => ({ ...f, vendor_number: 'VND-0001' }))
      }
    }
    suggestVendorNumber()
  }, [])
  ...
  return (
    <div className="dashboard-page">
      ...
      <main className="dashboard-main" style={{ maxWidth: 560 }}>
        <h1 style={{ margin: '0 0 32px', fontSize: '1.5rem' }}>Add OEM Supplier</h1>

        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div>
            <label style={labelStyle}>Vendor Number</label>
            <input style={inputStyle} value={form.vendor_number}
              onChange={e => setForm({ ...form, vendor_number: e.target.value })} required />
          </div>
          <div>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
            ...
            <button type="button" className="btn-signout" style={{ padding: '10px 18px' }} onClick={() => navigate('/admin/suppliers')}>
              Cancel
            </button>
          </div>
        </form>
