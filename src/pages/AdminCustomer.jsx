      ...
      <main className="dashboard-main">
        ...
        <div id="add-invoice" style={{ background: 'var(--slate)', border: '1px solid var(--border)', borderRadius: 10, padding: 24, marginBottom: 32 }}>
          ...
          <form onSubmit={handleAddInvoice}
            ...
            <div><label style={labelStyle}>Total $</label><input tabIndex={4} value={newInvoice.amount_total} onChange={e => setNewInvoice({ ...newInvoice, amount_total: formatCurrency(e.target.value) })} onBlur={e => { const val = e.target.value; setTimeout(() => setNewInvoice(v => ({ ...v, amount_total: finalizeCurrency(val) })), 0) }} required style={inputStyle} placeholder="0.00" /></div>
            <div><label style={labelStyle}>Paid $</label><input tabIndex={5} value={newInvoice.amount_paid} onChange={e => setNewInvoice({ ...newInvoice, amount_paid: formatCurrency(e.target.value) })} onBlur={e => { const val = e.target.value; setTimeout(() => setNewInvoice(v => ({ ...v, amount_paid: finalizeCurrency(val) })), 0) }} style={inputStyle} placeholder="0.00" /></div>
